...
const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('WebGL not supported for GlowCursor:', err);
      return;
    }
    ...
    try {
      ...
      console.warn('Failed to compile GlowCursor shader:', err);
      return;
    }
    ...
    const onWindowTouchMove = (event) => {
      ...
      if (
        ...
        const y = clamp(rect.height - (touch.clientY - rect.top), 0, rect.height);
        if (!initialized) initializeTrail(x, y);
        target.x = x;
        target.y = y;
        pointerInside = true;
    ...
    const render = (now) => {
      ...
      try {
        ...
        // Safe catch in case WebGL context is lost
      }
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className="glow-cursor__canvas pointer-events-none"
        style={{ mixBlendMode: blendMode }}
        aria-hidden="true"
      />
      {children && <div className="glow-cursor__content relative z-10 pointer-events-auto">{children}</div>}
    </div>
  );
};
